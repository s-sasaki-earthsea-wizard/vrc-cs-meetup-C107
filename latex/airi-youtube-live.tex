...
\subsubsection{データベースの既存インフラの活用と拡張}

\paragraph{AIRI本体の会話記憶システム}

AIRI本体は元々、Telegram Botでの\textbf{会話記憶}のためのテーブルを持っていました。
本プロジェクトでは既存のDBを拡張し、\textbf{Discordの投稿からレコードを登録する}ための新しいテーブルを追加しました：
...
\textbf{キャラクター設定としての知識}をDBとして管理するための追加実装を最小限に抑えることができました。
フォーク元のAIRIのコントリビューター様たちという巨人の肩の上にちょっとだけアクセサリを飾った、という気持ちです。
...
従来のプロンプト指示（「JSONで返して」）と異なり、
構造化されたスキーマに従った出力が保証されます。

\textbf{なぜJSON Modeを選んだか}:
\begin{itemize}
  \item \textbf{パースの確実性}: 正規表現やエラーハンドリングが不要
  \item \textbf{レイテンシ削減}: LLMが構造を考える時間が短縮される
  \item \textbf{コスト削減}: 出力トークン数が安定し、リトライが不要
\end{itemize}

これにより、キーワード抽出の失敗率がほぼゼロになりました。
...
ここでもJSONモードを利用することで、効率的な話題の選択を行うようにしています
