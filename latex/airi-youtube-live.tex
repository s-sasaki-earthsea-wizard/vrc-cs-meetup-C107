...
\renewcommand{\subsubsectionautorefname}{項}
...
こんにちは、さめと申します。
普段はフリーランスのソフトウェアエンジニアとして活動する傍ら、
YouTubeで科学技術動画を公開したり、
VRChatで物理学集会というイベントの主催をしています。

今回はRAG（LLMにユーザーの質問に応じて適切な資料等を検索して応答に反映させる仕組み）によって
キャラクター設定を豊かにしたAI VTuber (LLMと音声合成によってライブ配信を行うVTuber)
の開発の試行錯誤を赤裸々に記事にしました。

AIVTuberやRAGに興味がある方、個人開発で遊びたい方に読んでいただけたら嬉しいです。

執筆の機会を設けてくれた合同誌主催の夜鍋ヨナさまに深い感謝を申し上げます！

\begin{center}
  \includegraphics[width=0.8\columnwidth]{images/icon_circle.png}
\end{center}
...
\section{AI VTuberの個性をRAGで構築する - ソーシャルメディア投稿のナレッジDB化}
...
Neuro-sama\footnote{\url{https://www.twitch.tv/vedal987}}のようにゲームプレイ配信で個性的なキャラクターを演じるAI VTuberや、
プロンプトエンジニアリングを駆使したAI VTuberの実装も数多く生まれています
\footnote{AITuberを作ってみたら生成AIプログラミングがよくわかった件, \url{https://bookplus.nikkei.com/atcl/catalog/23/10/31/01079/}}\footnote{AITuberを作ってみたらプロンプトエンジニアリングがよくわかった件, \url{https://bookplus.nikkei.com/atcl/catalog/24/11/07/01683/}}。
また、AI VTuberの個性を豊かにする技術として\textbf{RAG（Retrieval-Augmented Generation、検索拡張生成）}も既に多くのプロジェクトで活用されています。
RAGは、データベースから関連情報を検索してLLMのプロンプトに注入することで、より文脈に沿った応答を生成できる技術です。
上記の書籍でもRAGについて詳しく解説されていますし、本記事で紹介するプロジェクトのフォーク元であるAIRI\footnote{\url{https://github.com/moeru-ai/airi}}でも既にRAGが実装されています。
では、本プロジェクトの独自性は何なのか？
...
開発期間は約1ヶ月（2025年10月から開発開始）。現在も開発途上で、月1回の配信を目標に機能追加を続けています。
...
\begin{figure*}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{images/aivtuber-streaming-screenshot.png}
...
なので、最初は「あまり詳しくない」という前置きから始まります\footnote{本システムでは、口調（フレンドリーな話し言葉）や基本的なプロフィール（職業や居住地など）をシステムプロンプトで定義しています。また、ナレッジDBに存在しない知識については「知らない」と正直に答えるよう指示しており、これによりLLMが架空の体験を捏造してキャラクターの一貫性を損なうことを防いでいます。詳細な趣味嗜好（宝塚、ポケモン等）はRAGで実現しています。}：
...
\begin{figure*}
  \centering
  \includegraphics[width=0.9\textwidth]{images/ai-vtuber-system-architecture.pdf}
...
\begin{enumerate}
  \item [46.2\%] 好きな漫画のひとつは「北斗の拳」！〜好きなキャラはトキ！ ← \twemoji{cross mark} 無関係だが類似度高い。「トキ」を鳥の朱鷺と勘違いしている？
  \item [45.9\%] ポケモンのダブルバトルが面白い ← \twemoji{thinking face} 微妙（架空の生き物）
  \item [42.2\%] 実家には犬がいる。16歳のヨークシャーテリア ← \twemoji{white_check_mark} 本命だが3位
\end{enumerate}
...
\begin{itemize}
  \item \twemoji{white_check_mark} 形態素解析不要 (文字列の分割)
  \item \twemoji{white_check_mark} 同義語に対応（"ポケモン" = "Pokémon"）
  \item \twemoji{white_check_mark} 意味的な関連性を捉える
  \item \twemoji{white_check_mark} 多言語対応
\end{itemize}

しかし、以下の課題があります：

\begin{itemize}
  \item \twemoji{cross mark} 文脈を考慮しない（架空と現実の区別がつかない）
  \item \twemoji{cross mark} ユーザーの意図を理解できない（ペットの話を聞きたいのか、キャラクターの話を聞きたいのか）
  \item \twemoji{cross mark} 類似度スコアの絶対値に意味がない（46.2\%と42.2\%の差が重要かは文脈次第）
\end{itemize}
...
この問題を解決するため、\autoref{fig:three-stage-pipeline}に示す3段階パイプラインを実装しました
\footnote{Cohere Rerankのように質問と関連した文書の検索を最適化するモデルも提供されています。実装時に存在を知らなかったので、いずれ試してみたいと考えています。}。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\columnwidth]{images/three-stage-pipeline.pdf}
  \caption{3段階検索パイプライン - クエリ拡張・ベクトル検索・再ランキング}
...
\begin{quote}
  再ランキング後のTop-K:
  \begin{enumerate}
    \item 「実家には犬がいる。16歳のヨークシャーテリア」 ← \twemoji{white_check_mark}正解！
  \end{enumerate}
...
\autoref{subsec:overview}で紹介した通り、応答内容で再度検索することで話題を深掘りします（\autoref{fig:iterative-search-flow}）。

\begin{figure}
  \centering
  \includegraphics[width=0.8\linewidth]{images/iterative-search-flow.pdf}
  \caption{反復的RAG検索による話題の深掘り}
  \label{fig:iterative-search-flow}
\end{figure}
...
なお、本記事で紹介したRAGの実装は、かなり低レイヤーから構築したものですが、
現在ではGoogleのGemini APIにおけるFile Search Tool\footnote{\url{https://blog.google/technology/developers/file-search-gemini-api/}}をはじめ、
RAGを簡単に利用できる有力なサービスが多数提供されています。
本記事の内容もすぐに古くなってしまうことを確信しています。

それでも、実際に手を動かして試行錯誤した記録を残しておくことには、
実装の裏側や設計判断の背景を知る上で価値があると考え、
この形で記事にすることにしました。
...
\subsection*{Appendix: ゲームプレイ機能への挑戦}

フォーク元のAIRIプロジェクトには、既にマインクラフトをプレイする機能が実装されています。
これに注目して自分で別のゲームを遊ばせる機能を実装することを考えました。
しかし、マインクラフトのような一般的なゲームは画像認識や空間把握、
記憶等のリソース管理など、
AI VTuberに遊ばせるために解決すべき技術課題が非常に多くなってしまいます
\footnote{OpenAIやAnthropic、Googleなどの企業が初代ポケモンをAIに攻略させるプロジェクトに挑戦していたことは記憶に新しいと思います。}。

そこで、より記号的でテキストベースでの解釈が可能な将棋に着目しました。
将棋を選んだことは以下のように複合的な理由からです。

\begin{itemize}
  \item 盤面や指し手がすべてテキストで表現可能であり、画像処理が不要なこと
  \item ルールを知っている人が多く、視聴者が配信内容を理解しやすい
  \item YouTubeには実際に将棋の実況や解説を行っている配信者が多数存在することから需要が存在し、成功した場合に視聴数やチャンネル登録者数の増加を見込めること
  \item 既存の将棋ソフトウェアが豊富で技術的な連携がしやすいこと、特に一部のソフトウェアは強さを調整できるため、よりユーザーフレンドリーな配信を実現できること
  \item 筆者自身がアマチュア初段の棋力を持っており、指し手や形勢の解説の妥当性を検証できること
\end{itemize}


現在試みているアプローチは、将棋ソフト同士の対局において、
片方の指し手を「AIアバター自身が考えて指した手」と認識させるプロンプトを注入することです。
ソフト同士の対局を選んだ理由も複数あります。

\begin{itemize}
  \item 多くのネット対局サービスではソフト指し（人間がソフトの指示通りに指すこと）が禁止されているか、ソフトであることを明記するルールが設けられており、人間との対局には制約があること
  \item 現在の最強の将棋ソフトは人間の名人でも勝てないレベルの強さを持つため、そのまま対局させても視聴者にとって理解が困難な配信となってしまうこと
  \item ソフトの強さをアマチュア初段程度に調整することで、視聴者も筆者も指し手の意味や形勢を理解できる、配信としての面白さを持つ対局を実現できること
\end{itemize}

これにより、AIアバターがあたかも自分で将棋を考えて指しているかのような実況配信を実現しようとしています。
まだ実験段階であり、うまくいくかどうかは分かりませんが、
技術的な実現可能性と配信コンテンツとしての面白さの両立を目指して試行錯誤を続けています。
...
個人開発の記録を記事にまとめる作業は想像以上に大変でした。
「なぜこの設計にしたのか？」「他の選択肢はなかったのか？」と
自分に問いかけたり、改めて関連技術の調査をすることになり、
改めて実装の粗や先行研究調査の不足が見えてきます。

今回、合同誌に寄稿させていただいて特に楽しかったのは、
\textbf{論文や技術記事とは違って、失敗談や試行錯誤を正直に書ける}ことでした。
「最初は失敗したけど、こう工夫したら解決した」という生々しい試行錯誤の過程を残せることが
同人誌や個人開発の醍醐味であり、読者にとっても参考になるのではないかと考え、
このような形の記事を寄稿しました。

本プロジェクトはMITライセンスでGitHub上に公開していますので、
バグレポート、機能提案、プルリクエストなど、すべて歓迎です。

執筆の機会を与えてくださった合同誌主催の夜鍋ヨナさん、
フォーク元のAIRIプロジェクトの開発者の皆様、
そして何より、ここまで読んでくださった皆様に、心より感謝申し上げます。
